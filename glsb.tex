...
\epsscale{1}
\caption{ Comparison of the GLSB disks with other galaxy disks from the literature.  THINGS data taken from \citet{DeBlok2008}, Malin 1 from \citet{Galaz15}, Malin 2 and UGC 6614 from \citet{Pickering97}.  Here, $R_{max}$ is the maximum radius with an \HI\ detection for the THINGS galaxies, Malin~2, and UGC~6614 and the maximum radius with a stellar flux detection for Malin~1 XXX.  These are all maximum detection radii, and thus should be considered lower limits.  XXX--add in the stellar disks for Malin 2 and UGC 6614. \citet{Kasparova14} trace the light out to 60\arcsec on Malin~2, so 56 kpc\label{disk_comp}.  I trace UGC 6614 out to 65\arcsec, so 29 kpc.}
\end{figure*}
...
%Some relevant papers for comparison:
