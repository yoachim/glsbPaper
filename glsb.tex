...
The disks in GLSB systems are incredibly large, with Malin~1 being detected to a radius of over 100 kpc \citep{Moore06}.  Disks of this size are a challenge for typicall $\Lambda$CDM cosmology galaxy formation and exotic formation scenarios have been proposed for GLSB systems.  \citet{Mapelli08} form GLSB-like galaxies via the fading of collisional ring galaxies xxx-expand this a bit.  In their model, a collision where a thingy hits thuswise, blah blah, sets off an expanding ring of star formation.  As this the star formation subsides and the stetllar population ages, it fades to form a disk that resembles in size and exponential profile observed GLSBs.  XXX--also tie in \citep{Mapelli12}.



XXX-add discussion of possibility the centers of GLSBs are normal galaxies.

Points to cover in intro
\begin{itemize}
\item{GLSB galaxies are really big spirals}
\item{Typically classified as extreme spirals observaionally}
\item{but HST observations of Malin~1 show it is a regular galaxy with a distinct extended disk \citet{Barth07}}
\item{Models for GLSB formation}
  \begin{itemize}
  \item{regular growth (maybe like polar ring galaxies, or just the large end of spirals)}
  \item{faded collisional ring. \citet{Mapelli08, Mapelli12} }
  \item{high angular momentum accretion event. \citet{Pen06}}
  \end{itemize}
  \item{Discuss how, if GLSBs are merger remnents, they represent a unique possibility to trace the dark matter halo of elliptical galaxies.  }
\end{itemize}
...
GLSBs are typically classified as spiral galaxies \citep{Das13}, however, high resolution imaging of Malin~1 has shown that the central region appears to be a normal 
...
\epsscale{1}
\caption{ Comparison of the GLSB disks with other galaxy disks from the literature.  THINGS data taken from \citet{DeBlok2008}, Malin 1 from \citet{Moore06}, Malin 2 and UGC 6614 from \citet{Pickering97}.  Here, $R_{max}$ is the maximum radius with an \HI\ detection for the THINGS galaxies and the maximum radius with a stellar flux detection for the GLSB galaxies.  It is interesting to note that the stellar disks of GLSB galaxies are often larger than the \HI\ disks of nearby systems.  \label{disk_comp}}
\end{figure}
...
We have observed two giant low surface brightness galaxies with the VIRUS-P IFU.  We find the systems are best explained as massive elliptical galaxies which have managed to accrete stars and gas into a large disk.  The stellar stars have a disk-like velocity field, and are dominated by old stars, but do show signs of ongoing star formation.

In a future work, we plan to combine our IFU observations with higher spatial resolution long-slit spectra from the Apache Point 3.5m telescope to determine if the central ellipticals are dynamically distinct from their disks.  This data will also help up build dynamical models for the systems.
