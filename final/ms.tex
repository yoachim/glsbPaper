...
\begin{abstract}
We present IFU observations of the giant low surface brightness galaxies Malin~2 and UGC~6614.
...
We argue that Malin~2 and UGC~6614 are normal elliptical galaxies that have undergone high angular momentum mergers which formed their GLSB disks, making them unique systems for probing the dark matter halos of elliptical systems.
...
Observers tend to classify GLSB systems as normal spirals.
...
While GLSB systems superficially resemble normal spiral galaxies, they are clearly extreme outliers when their scale lengths are compared to other galaxy samples, being around a factor of 2 larger than the largest normal spirals \citep[Figure~4]{Dalcanton97}. 
...
The other popular formation scenario for GLSB systems is that they are normal galaxies that undergo a high angular momentum minor merger where the LSB disk is the shredded remains of an accreted galaxy.
...
The rest taken from SDSS. Stellar masses calculated from \citet{Conroy09}}
\end{deluxetable}
...
The Mitchell Spectrograph with the VP-2 IFU bundle used here has a square array of 246 optical fibers which sample the 1.9\arcmin$\times$1.9\arcmin\ field-of-view with a 1/3 filling factor, with each fiber having a diameter of 4.23\arcsec.
...
The corners of the IFU array contain enough empty fibers to measure the sky background in each exposure.
...
We use GANDALF to measure the [\ion{O}{2}] doublet, [\ion{O}{3}] doublet, and the higher order Balmer lines \hb, H$\gamma$, H$\delta$, H$\epsilon$ while masking the night skyline at 5577 \AA.
...
Bottom Right:  The relation between central velocity dispersion and the Mg2 absorption feature.
 From the point of view of the SDSS data reduction pipeline, GLSB galaxies appear as relatively normal red elliptical galaxies.
...
This seems to point against the GLSB formation mechanism of \citet{Mapelli08}, as we do not observe \HI\ disks in normal spirals large enough to convert into GLSB systems unless an interaction is able to also boost the angular momentum of the resulting stellar disk.
...
As we have shown in \S\ref{sec:centers} and \S\ref{sec:disks}, GLSB galaxies can be well described as large normal ellipticals that have managed to accrete a high angular momentum disk of stars and gas.
...
\citet{Murphy2011} are able to use IFU observations of M87 to reach $\sim2 R_e$ using just stellar data, and then rely on globular cluster kinematics to extended out to $5 R_e$ where they find a dark matter fraction of 85\%.
\citet{Weij09} is similarly able to use IFU observations to reach $\sim 4 R_e$ on two elliptical galaxies while \citet{Proctor09} reaches $\sim 3 R_e$ using multi-object spectroscopy.
...
It also seems unlikely that GLSBs are just extreme versions of normal disk galaxies \citep{Das13}.
...
This is a very sharp gradient compared to what has been found in normal spirals for the bulge-to-disk transition region.
...
We have observed two giant low surface brightness galaxies with the VIRUS-P IFU.
...
\begin{figure}
\plottwo{sdssbinned_Malin2.eps}{Malin2mu.eps}
\epsscale{.5}
\plotone{Malin2profile_fit.eps}
\epsscale{1}
...
The inner bins show strong Mg, Fe, and Ca absorption features, while the outer bins have \hb\ and \ion{O}{3} emission lines. \label{malin2_binmap}}
\end{figure}


\begin{figure}
\epsscale{.5}
\plotone{Malin2_chibar_sfr.eps}
\epsscale{1}
...
\begin{figure}
\plottwo{sdssbinned_UGC06614.eps}{UGC06614mu.eps}
\epsscale{0.5}
\plotone{UGC06614profile_fit.eps}
\epsscale{1}
...
\caption{Left:  Map showing which fibers were binned for UGC~6614.
Right:  The binned spectra, color-coded to match the map on the left.
As with Malin~2, the inner spectra show strong metallicity absorption features while the outer spectra show emission lines. \label{ugc_binmap}}
\end{figure}
...
\begin{figure}
\epsscale{.5}
\plotone{UGC06614_chibar_sfr.eps}
\caption{Best fitting star formation histories.
For each bin, the SFH is normalized by the peak bin.
Error bars are based on $\Delta\chi^2=100$ or the 4 best fitting models, whichever provides more points.
As with Malin~2, old stars dominate at all radii.
\label{sfh_ugc} }
\epsscale{1}
\end{figure}
...
\begin{figure}
\epsscale{.5}
\plotone{UGC06614_chibar_z.eps}
\epsscale{1}
\caption{Best fitting stellar metallicities for the binned UGC~6614 spectra.
Dashed line shows the maximum model value.
\label{metal_ugc}}
\end{figure}
...
\caption{Rotation curves extracted from the best fitting model in Fig~\ref{ugc_kinem_fibers}.
The dashed curve shows the expected stellar mass contribution to the rotation curve.
Vertical lines mark $R_e$ and $5R_e$.
Right panel includes \HI\ data from \citet{Pickering97}. \label{ugc_kinem}}
\end{figure*}
